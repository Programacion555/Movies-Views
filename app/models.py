...
class Movie:
    ...
    def save(self):
        ...
        cursor = db.cursor()
        if self.id_movie:
            cursor.execute(""" UPDATE movies SET title =%s, director = %s, release_date =%s, banner =%s WHERE id_movie = %s""",(self.title,self.director,self.release_date,self.banner,self.id_movie))
        else:
